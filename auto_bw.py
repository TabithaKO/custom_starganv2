...
imdir = './stargan_data/black_women/'
...
file_list = os.listdir(imdir)  #[:15000]

#[files.append(img_prep(f)) for f in os.listdir(imdir)[:15000]]
...
#files =  list(map(img_prep,file_list) )
...
autoencoder.fit(train_input[0:9000], train_input[0:9000], batch_size=16, epochs=200, validation_split=0.1)
autoencoder.save("bw_autoencoder_model.model")

# from google.colab.patches import cv2_imshow
#%%
# test and save some random images.
rng = default_rng()
file_list = os.listdir('./stargan_data/black_women/')  
...
for i,test_img in enumerate(test_imgs):
    ...
    cv2.imwrite(f"bw_autoencoder_test_{i}_{test_img}", ae_out)
