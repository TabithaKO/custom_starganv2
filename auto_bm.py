...
autoencoder.fit(train_input[0:8000], train_input[0:8000], batch_size=16, epochs=200, validation_split=0.1)
autoencoder.save("bm_autoencoder_model.model")

# test and save some random images.
rng = default_rng()
file_list = os.listdir('./stargan_data/black_men/')  
...
for i,test_img in enumerate(test_imgs):
    ...
    cv2.imwrite(f"bm_autoencoder_test_{i}_{test_img}", ae_out)
